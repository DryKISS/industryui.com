import { isValid, differenceInYears, format as format$1 } from 'date-fns';
import bcrypt from 'bcryptjs';
import jwt from 'jsonwebtoken';
import moment from 'moment';
import React, { useContext, useRef, useDebugValue, createElement, useEffect, memo, forwardRef, useState } from 'react';
...
var dateFns = function dateFns(string) {
  return format$1(new Date(string), 'LLL d, H:mm');
};
var shortDate = function shortDate(string) {
  var date = Date.parse(string);
  return !isNaN(date) ? format$1(date, 'MMM d') : string;
};
...
var DetailsText = function DetailsText(_ref) {
  var content = _ref.content,
      text = _ref.text;
  return __jsx$d(React.Fragment, null, __jsx$d(Space, {
    marginBottom: "sm"
  }), __jsx$d(Text, {
    content: content,
    context: "dark",
...
var Link = function Link(_ref) {
  ...
  return !validatorUri(to) ? __jsx$l(NextLink, _extends({}, obj, {
    passHref: passHref,
    replace: replace,
    scroll: scroll,
    shallow: shallow
...
export { ALIGN, Address, Avatar, BACKGROUND, Badge, Blockquote, Button, ButtonToolbar, COLOUR, COMMON_INPUT_STYLES, CONTEXT, Close, Column, Container, DIMENSION, DIMENSION_PROP_TYPES, DISPLAY, DISPLAY_PROP_TYPES, Date$1 as Date, Details, DetailsText, Divider, ERROR_STYLE, FONTSIZE, Figure, GoogleAnalyticsPageView, GoogleEvent, Heading, ICON_PREFIX, ICON_PULL, ICON_SIZE, Icon, IconStacked, Image$1 as Image, Link, List, ListItem, MEDIA_QUERY, MEDIA_QUERY_MAX, Position, Progress, ProgressBar, ProgressBarDiv, RandomColor, RandomKey, ReactHolderJs, ResizeDetector, Row, SIZE, SPACER, SPACER_FORMULA, SPACER_PROP_TYPES, SPACING, Space, StyledLink, Switch, Text, Theme, ThemeStyle, Toggle, Tooltip, Truncate, TruncateByMaxHeight, age, arrayOfValues, averageGeolocation, blendLinearRgb, camelCase, capitalize, colourList, convertImgUrlToDataURLviaCanvas, dateFns, debounce, decodeToken, filterByKey, filterByString, findByKey, formatDate, formatDateStandard, formatIntDate, formatIntDateShort, formatIntDateYear, formatPrice, formatRelativeTime, formatTime, generateToken, getAcronym, getAssociations, getFirst, getItemAssociation, getItemAssociations, getLast, getManyToManyAssociations, getOne, getUrlParameter, getUserFromToken, hashPassword, historyPush, mergeLocalData, parsePostCode, requestSimulator, shadeLinearRgb, shortDate, sizeArray, slugify, useTooltip, validatePassword, validateToken, validatorPostCode, validatorUri, validatorUuid4, viewPort };
